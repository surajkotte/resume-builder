...
## Core state fields (`MainGraphState`)

| Field | Type | Set by |
|---|---|---|
| `user_message` | `str` | caller |
| `messages` | `Annotated[list, add_messages]` | accumulates across turns for chat continuity |
| `resume_url` | `Optional[str]` | caller (also passed via `UserContext`) |
| `job_description` | `Optional[str]` | JD-extraction step / user message |
| `resume_markdown` | `Optional[str]` | `resume_extraction` |
| `github_readmes` | `Optional[str]` | `github_readme` |
| `new_resume_markdown` | `Optional[str]` | `resume_creation` |
| `atc_score` | `Optional[int]` | `atc_checker` |
| `skill_gap` | `Optional[SkillGapAnalysis]` | `skill_gap_analyzer` |
| `intent` | `Optional[str]` | `intent_router` |
| `chat_response` | `Optional[str]` | `general_chat` |
| `number_of_retries` | `Optional[int]` | `resume_creation` |

`UserContext` (runtime, not state): `username`, `api_key` (GitHub), `resume_url`.

---

## Design notes worth keeping visible

- **Checkpointer vs. Store**: checkpointer = automatic, per-step snapshot of graph
  state scoped to one `thread_id` (resume/replay/recover a run). Store = manually
  written key-value data scoped across threads (e.g. per `user_id`), for durable
  memory like "this candidate prefers metric units" or facts from a prior session.
- **UI reads**: the Next.js app should not read checkpoint internals directly.
  Either expose a small `GET /threads/{thread_id}/messages` endpoint that unpacks
  the checkpoint's `messages` channel, or maintain a denormalized `messages`
  table written alongside the graph run for fast pagination.
- **Tool argument size**: agent tools avoid taking large text (resume, JD) as
  LLM-filled arguments — they close over `state`/`runtime` instead, since
  smaller models are prone to silently truncating tool orchestration when asked
  to reproduce long strings as function-call arguments.
- **Retry loop bound**: `condition_check` caps retries at 3 to prevent runaway
  `resume_creation` ↔ `atc_checker` loops on a stubborn low score.
